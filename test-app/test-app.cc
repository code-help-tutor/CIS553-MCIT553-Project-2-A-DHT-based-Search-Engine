  ...
  static TypeId tid
      = TypeId ("TestApp")
            ...
            .AddAttribute ("AppPort", "Listening port for Application", UintegerValue (10000),
                           MakeUintegerAccessor (&TestApp::m_appPort), MakeUintegerChecker<uint16_t> ())
            .AddAttribute ("PingTimeout", "Timeout value for PING_REQ in milliseconds", TimeValue (MilliSeconds (2000)),
                           MakeTimeAccessor (&TestApp::m_pingTimeout), MakeTimeChecker ())
            .AddAttribute ("TrafficPeriod", "Traffic Period in milliseconds", TimeValue (MilliSeconds (2)),
                           MakeTimeAccessor (&TestApp::m_trafficPeriod), MakeTimeChecker ());

  ;
  return tid;
}

TestApp::TestApp () : m_auditPingsTimer (Timer::CANCEL_ON_DESTROY)
{
  double min = 0x00000000;
  ...
  if (destAddress != Ipv4Address::GetAny ())
    {
      ...
      TRAFFIC_LOG ("Sending PING_REQ to Node: " << nodeNumber << " IP: " << destAddress << " Message: " << pingMessage
  ...
  for (iter = m_pingTracker.begin (); iter != m_pingTracker.end ();)
    {
      ...
      if (pingRequest->GetTimestamp ().GetMilliSeconds () + m_pingTimeout.GetMilliSeconds ()
