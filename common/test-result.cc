...
int entry_count;
bool is_enabled;
bool is_grading_enabled;
std::string test_name;
char error[100][100];
int error_count;

void
openResultFile(std::string fileName)
  {
  if (fileName == "")
    is_enabled = false;
  else
...
void
openGraderResultFile(std::string fileName)
  {
  if (fileName == "")
    is_grading_enabled = false;
  else
    is_grading_enabled = true;
  if (!is_grading_enabled)
    return;
...
endDump()
  ...
  if (is_grading_enabled) {
    std::string result = error_count == 0 ? "correct\n\n" : "wrong";

    ofs << test_name << " is " << result;
    if (result == "wrong") {
      if (error_count == 1)
        ofs << ", with " << error_count << " error\n\n";
      else
        ofs << ", with " << error_count << " errors\n\n";
      }
    for (int i = 0; i < error_count; i++)
      ofs << "error " << i + 1 << ": " << error[i] << "\n";
    }
  else {
...
endDumpTrafficTrace()
  ...
  if (entry_count < entry_total)
    ...
    error_count = 1;
    sprintf(error[0],
      "there is less traffic trace than expected in standard "
      "answer (%i)\n",
      entry_total);
    }
...
void
checkTrafficTrace(std::string g_nodeId, std::string g_moduleName)
  {
  if (!is_enabled)
    return;
  ...
  if (entry_count >= entry_total)
    ...
    error_count = 1;
    sprintf(error[0],
      "there is more traffic trace than expected in standard "
      "answer (%i)\n",
      entry_total);
    return;
    }
...
void
checkNeighborTableEntry(uint32_t neighborNum, Ipv4Address neighborAddr, Ipv4Address ifAddr)
  {
  if (!is_enabled)
    return;
  entry_count++;
  ...
  if (iter == m_neighborTable.end())
    {
    sprintf(error[error_count++], "neighbor %i is not found in standard answer", neighborNum);
    return;
    }
  NeighborTableEntry entry = iter->second;
  if (!(neighborAddr == entry.neighborAddr))
    {
    sprintf(error[error_count++], "for neighbor %i, neighbor address doesn't match\n", neighborNum);
    return;
    }
  if (!(ifAddr == entry.interfaceAddr))
    {
...
void
checkNeighborTableEntry(std::string neighborNum, Ipv4Address neighborAddr, Ipv4Address ifAddr)
  {
  uint32_t num;
...
void
checkRouteTableEntry(std::string dstNum, Ipv4Address dstAddr, uint32_t nextHopNum, Ipv4Address nextHopAddr,
...
// Note: It is OK to output route to destination that is different from
// standard answer, as long as it gives the minimum cost.

void
checkRouteTableEntry(uint32_t dstNum, Ipv4Address dstAddr, uint32_t nextHopNum, Ipv4Address nextHopAddr,
  Ipv4Address ifAddr, uint32_t cost)
  {
  if (!is_enabled)
    return;
  ...
  if (iter == m_routingTable.end())
    {
    sprintf(error[error_count++], "route to %i is not found in standard answer", dstNum);
    return;
    }
  RoutingTableEntry entry = iter->second;
  if (!(dstAddr == entry.destAddr))
    {
  ...
  if (nextHopNum != entry.nextHopNum)
    {
    sprintf(error[error_count++],
      "for route to %i, next hop number doesn't match. But it "
      "gives the same minimum cost\n",
      dstNum);
    return;
    }
  if (!(nextHopAddr == entry.nextHopAddr))
    {
